...
class RXArmThread(QThread):
    ...
    def callback(self, data):
        ...
        self.updateEndEffectorReadout.emit(self.get_ee_pose())
    ...
    def get_ee_pose(self):
        ...
        Gst_ref = np.array([[1, 0, 0, 0], [0, 1, 0, L3 + L4 + L5 + L6], [0, 0, 1, L0 + L1 + L2], [0, 0, 0, 1]])

        twist1 = np.array([[0], [0], [0], [0], [0], [1]])
        twist2 = np.array([[0], [-(L0+L1)], [0], [-1], [0], [0]])
        twist3 = np.array([[0], [L0+L1+L2], [-L3], [1], [0], [0]])
        twist4 = np.array([[0], [L0+L1+L2], [-(L3+L4)], [1], [0], [0]])
        twist5 = np.array([[-(L0+L1+L2)], [0], [0], [0], [1], [0]])

        Twists = np.array([twist1, twist2, twist3, twist4, twist5])

        pose = FK_pox(self.rxarm.get_positions(), Gst_ref, Twists)
        #print((IK_geometric(pose)[1]*180)/(np.pi))
        return list(pose)
